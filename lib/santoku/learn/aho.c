...
static const char tk_aho_c3[64] = {
  'a','a','a','a','a','a','a','c',
  'e','e','e','e','i','i','i','i',
  'd','n','o','o','o','o','o', 0,
  'o','u','u','u','u','y', 0, 's',
...
static const char tk_aho_c4[64] = {
  'a','a','a','a','a','a','c','c',
  'c','c','c','c','c','c','d','d',
  'd','d','e','e','e','e','e','e',
  'e','e','e','e','g','g','g','g',
...
static const char tk_aho_c5[64] = {
  'l','l','l','n','n','n','n','n',
  'n','n','n','n','o','o','o','o',
  'o','o','o','o','r','r','r','r',
  'r','r','s','s','s','s','s','s',
...
static int64_t tk_aho_normalize (const char *in, size_t len, uint8_t *out, int64_t *pm) {
  int64_t j = 0;
  for (size_t i = 0; i < len; ) {
    ...
    if (c < 0x80) {
      ...
      const char *tbl = (c == 0xC3) ? tk_aho_c3 : (c == 0xC4) ? tk_aho_c4 : tk_aho_c5;
...
static tk_aho_scan_result_t tk_aho_scan (
  ...
  #pragma omp parallel
  {
    ...
    for (int ti = 0; ti < n_texts; ti++) {
      ...
      int64_t nlen = tk_aho_normalize(texts[ti], tlen, norm_buf, pm_buf);
...
static int tk_aho_create_lua (lua_State *L)
{
  ...
  for (int64_t p = 0; p < n_patterns; p++) {
    ...
    int64_t nlen = tk_aho_normalize(pat, len, norm_buf, NULL);
    int32_t cur = 0;
