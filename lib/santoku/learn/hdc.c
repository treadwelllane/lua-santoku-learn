...
#define TK_HDC_N_CHARS 28

static inline int tk_hdc_char_idx (unsigned char c)
{
...
static inline int64_t tk_hdc_pack_ngram (const char *text, size_t pos, int64_t ng)
{
  ...
  for (int64_t k = 0; k < ng; k++) {
    h ^= (uint64_t)(unsigned)tk_hdc_char_idx((unsigned char)text[pos + (size_t)k]);
    h *= 0x100000001b3ULL;
  }
  return (int64_t)h;
}
...
{
  for (int c = 0; c < TK_HDC_N_CHARS; c++) {
...
{
  uint32_t wmap_end = weight_map ? tk_dumap_end(weight_map) : 0;
  size_t d_bytes = (size_t)d * sizeof(double);
  size_t dw_bytes = (size_t)d_words * sizeof(uint64_t);
  memset(out, 0, (uint64_t)n_samples * (uint64_t)d * sizeof(double));
  #pragma omp parallel
  {
    ...
    for (int64_t s = 0; s < n_samples; s++) {
      ...
      if ((int64_t)len >= ngram) {
        ...
        if (weight_map) {
          if (n_pos > pos_w_cap) {
            free(pos_w);
            pos_w = (double *)malloc(n_pos * sizeof(double));
            pos_w_cap = n_pos;
          }
          for (size_t i = 0; i < n_pos; i++) {
            int64_t packed = tk_hdc_pack_ngram(text, i, ngram);
            uint32_t iter = tk_dumap_get(weight_map, packed);
            pos_w[i] = (iter != wmap_end) ? tk_dumap_val(weight_map, iter) : 0.0;
          }
        }
        for (size_t i = 0; i < n_pos; i++) {
          double weight = pos_w ? pos_w[i] : 1.0;
          if (weight == 0.0) continue;
          int ci0 = tk_hdc_char_idx((unsigned char)text[i]);
          int64_t sh0 = ngram - 1;
          const uint64_t *a0 = shifted_base + ((uint64_t)sh0 * TK_HDC_N_CHARS + (uint64_t)ci0) * (uint64_t)d_words;
          memcpy(tmp, a0, dw_bytes);
          for (int64_t k = 1; k < ngram; k++) {
            int ck = tk_hdc_char_idx((unsigned char)text[i + (size_t)k]);
            int64_t shk = ngram - 1 - k;
            const uint64_t *ak = shifted_base + ((uint64_t)shk * TK_HDC_N_CHARS + (uint64_t)ck) * (uint64_t)d_words;
...
static int tk_hdc_reweight_lua (lua_State *L)
{
  ...
  tk_dvec_t *weights = tk_dvec_peek(L, 4, "weights");
...
static void tk_hdc_init_cached (tk_hdc_t *h)
{
  ...
  size_t base_bytes = (size_t)TK_HDC_N_CHARS * (size_t)dw * sizeof(uint64_t);
  uint64_t *base = (uint64_t *)tk_hdc_amalloc(base_bytes);
  tk_hdc_gen_base_packed(base, h->d, dw, h->hdc_seed);
  size_t shifted_bytes = (size_t)h->hdc_ngram * (size_t)TK_HDC_N_CHARS * (size_t)dw * sizeof(uint64_t);
  h->shifted_base = (uint64_t *)tk_hdc_amalloc(shifted_bytes);
  for (int64_t sh = 0; sh < h->hdc_ngram; sh++) {
    for (int c = 0; c < TK_HDC_N_CHARS; c++) {
      uint64_t *src = base + c * dw;
      uint64_t *dst = h->shifted_base + (sh * TK_HDC_N_CHARS + c) * dw;
      if (sh == 0)
...
static int tk_hdc_create_lua (lua_State *L)
{
  ...
  lua_getfield(L, 1, "texts");
  if (!lua_istable(L, -1))
    return luaL_error(L, "create: texts required");
  int texts_idx = lua_gettop(L);
  ...
  if (!lua_isnil(L, -1)) {
    ...
    tk_ivec_t *wids = tk_ivec_peek(L, -1, "weight_ids");
    lua_getfield(L, 1, "weights");
    tk_dvec_t *wts = tk_dvec_peek(L, -1, "weights");
    weight_map = tk_hdc_build_weight_map(L, wmap, wids, wts);
...
static int tk_hdc_persist_lua (lua_State *L) {
  ...
  uint8_t has_weights = h->weight_map ? 1 : 0;
...
static int tk_hdc_load_lua (lua_State *L)
{
  ...
  if (version != 5) {
    tk_lua_fclose(L, fh);
    return luaL_error(L, "unsupported hdc version %d (expected 5)", (int)version);
  }
  int64_t d, hdc_ngram;
  uint64_t hdc_seed;
