...
typedef struct { int64_t tok; double val; uint8_t piv; } tk_pivot_entry_t;
#define tk_pivot_entry_lt(a, b) ((a).tok < (b).tok)
KSORT_INIT(tk_pivot_entry, tk_pivot_entry_t, tk_pivot_entry_lt)
  ...
  while (actual_landmarks < n_landmarks && !done) {
    ...
    if (is_csr) {
...

      {
        ...
        tk_pivot_entry_t *combined = (tk_pivot_entry_t *)malloc(
          total_pnnz * sizeof(tk_pivot_entry_t));
        uint64_t ci = 0;
        for (uint64_t b = 0; b < np; b++) {
          int64_t p = plo_arr[b];
          while (p < phi_arr[b]) {
            ...
            combined[ci].tok = tok;
            combined[ci].val = sum;
            combined[ci].piv = (uint8_t)b;
            ci++;
          }
        }
        ks_introsort_tk_pivot_entry(ci, combined);
        uint64_t cn = ci;
        #pragma omp parallel for schedule(static)
        for (uint64_t i = 0; i < n_docs; i++) {
          int64_t ii = csr_offsets[i], ihi = csr_offsets[i + 1];
          double kip_row[TK_CHOL_BLOCK];
          memset(kip_row, 0, np * sizeof(double));
          uint64_t cc = 0;
          while (ii < ihi && cc < cn) {
            int64_t dtok = csr_tokens[ii];
            int64_t ctok = combined[cc].tok;
            if (dtok < ctok) {
              ii++;
            } else if (dtok > ctok) {
              uint64_t step = 1;
              while (cc + step < cn && combined[cc + step].tok < dtok) step <<= 1;
              uint64_t glo = cc + (step >> 1), ghi = cc + step < cn ? cc + step : cn;
              while (glo < ghi) {
                uint64_t mid = glo + (ghi - glo) / 2;
                if (combined[mid].tok < dtok) glo = mid + 1; else ghi = mid;
              }
              cc = glo;
            } else {
              double doc_sum = 0.0;
              while (ii < ihi && csr_tokens[ii] == dtok) { doc_sum += csr_values[ii]; ii++; }
              while (cc < cn && combined[cc].tok == dtok) {
                kip_row[combined[cc].piv] += doc_sum * combined[cc].val;
                cc++;
              }
            }
          }
...
static inline int tm_encode (lua_State *L) {
  ...
  for (int64_t j = 0; j < im; j++) {
    double *col = full_chol + (uint64_t)j * nc;
    double s = 0.0, ss = 0.0;
    ...
    for (uint64_t i = 0; i < nc; i++)
      col[i] = (col[i] - raw_mean[j]) * raw_istd[j];
  }
  ctx->adjustment = raw_mean;
  ctx->inv_sqrt_eig = raw_istd;

  double *row_norms = (double *)calloc(nc, sizeof(double));
  if (!row_norms) return luaL_error(L, "encode: out of memory");
  for (uint64_t j = 0; j < m; j++) {
    double *col = full_chol + j * nc;
    for (uint64_t i = 0; i < nc; i++)
      row_norms[i] += col[i] * col[i];
  }
  for (uint64_t i = 0; i < nc; i++) {
    double nrm = sqrt(row_norms[i]);
    row_norms[i] = nrm > 1e-12 ? 1.0 / nrm : 1.0;
  }
  #pragma omp parallel for schedule(static)
  for (int64_t j = 0; j < im; j++) {
    double *col = full_chol + (uint64_t)j * nc;
  ...
  int xtx_idx = 0, xty_idx = 0, cm_idx = 0, ym_idx = 0, lc_idx = 0;
  ...
  if (has_gram) {
    ...
    xtx_idx = lua_gettop(L);
    cblas_dsyrk(CblasColMajor, CblasLower, CblasTrans,
      (int)m, (int)nc, inv_n, full_chol, (int)nc, 0.0, xtx->a, (int)m);
    for (uint64_t i = 0; i < m; i++)
      for (uint64_t j = i + 1; j < m; j++)
        xtx->a[j * m + i] = xtx->a[i * m + j];

    uint64_t unl = (uint64_t)nl;
    tk_dvec_t *xty = tk_dvec_create(L, m * unl, 0, 0);
    xty->n = m * unl;
    xty_idx = lua_gettop(L);
    memset(xty->a, 0, m * unl * sizeof(double));

    tk_dvec_t *ym = tk_dvec_create(L, unl, 0, 0);
    ym->n = unl;
    ym_idx = lua_gettop(L);
    memset(ym->a, 0, unl * sizeof(double));
    ...
    if (has_gram_labels) {
      ...
      free(pos);
      for (int64_t l = 0; l < nl; l++)
        ym->a[l] = (double)(csc_off[l + 1] - csc_off[l]) * inv_n;
      #pragma omp parallel for schedule(static)
      ...
      cblas_dgemm(CblasRowMajor, CblasNoTrans, CblasNoTrans,
        (int)m, (int)nl, (int)nc, inv_n, full_chol, (int)nc,
        gram_targets->a, (int)nl, 0.0, xty->a, (int)nl);
      for (uint64_t i = 0; i < nc; i++)
        for (int64_t l = 0; l < nl; l++)
          ym->a[l] += gram_targets->a[i * (uint64_t)nl + (uint64_t)l] * inv_n;
    }
  }
  ...
  if (has_gram) {
    ...
    if (lc_idx) lua_pushvalue(L, lc_idx); else lua_pushnil(L);
    return 9;
  }
  return 4;
}
...
static inline int tk_nystrom_encoder_load_lua (lua_State *L) {
  ...
  if (version != 14) {
    tk_lua_fclose(L, fh);
    return luaL_error(L, "unsupported nystrom encoder version %d (expected 14)", (int)version);
  }
  uint8_t dense_mode;
...
static inline int tk_spectral_gram_lua (lua_State *L) {
  ...
  double *cm_parts = (double *)calloc((uint64_t)nt * (uint64_t)d, sizeof(double));
  #pragma omp parallel
  {
    int tid = omp_get_thread_num();
    double *local = cm_parts + (uint64_t)tid * (uint64_t)d;
  ...
  for (int64_t j = 0; j < d; j++) {
    double s = 0.0;
    for (int t = 0; t < nt; t++)
      s += cm_parts[(uint64_t)t * (uint64_t)d + (uint64_t)j];
    cm->a[j] = s * inv_n;
  }
  free(cm_parts);
  ...
  if (has_labels) {
    ...
    #pragma omp parallel
    {
      double *tmp = (double *)malloc((uint64_t)d * sizeof(double));
      #pragma omp for schedule(dynamic, 64)
      for (int64_t l = 0; l < nl; l++) {
        int64_t cnt = csc_off[l + 1] - csc_off[l];
        ym->a[l] = (double)cnt * inv_n;
        memset(tmp, 0, (uint64_t)d * sizeof(double));
        for (int64_t j = csc_off[l]; j < csc_off[l + 1]; j++)
          cblas_daxpy((int)d, 1.0, X + csc_rows[j] * d, 1, tmp, 1);
        for (int64_t k = 0; k < d; k++)
          xty->a[k * nl + l] = tmp[k] * inv_n;
      }
      free(tmp);
    }
    free(csc_off);
    ...
    cblas_dgemm(CblasRowMajor, CblasTrans, CblasNoTrans,
      (int)d, (int)nl, (int)n, inv_n, X, (int)d,
      targets->a, (int)nl, 0.0, xty->a, (int)nl);
    for (int64_t i = 0; i < n; i++)
      for (int64_t l = 0; l < nl; l++)
        ym->a[l] += targets->a[i * nl + l] * inv_n;
  }
