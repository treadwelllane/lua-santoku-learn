...
static inline int tm_encode (lua_State *L) {
  ...
  for (uint64_t j = 0; j < m; j++) {
    double s = 0.0;
    for (uint64_t i = 0; i < nc; i++) s += full_chol->a[i * m + j];
  ...
  lua_getfield(L, 1, "cca_csc_offsets");
  if (!lua_isnil(L, -1)) {
    tk_ivec_t *cca_off = tk_ivec_peek(L, -1, "cca_csc_offsets");
    lua_getfield(L, 1, "cca_csc_indices");
    tk_ivec_t *cca_ind = tk_ivec_peek(L, -1, "cca_csc_indices");
    lua_getfield(L, 1, "cca_weights");
    tk_dvec_t *cca_wt = tk_dvec_peek(L, -1, "cca_weights");
    lua_getfield(L, 1, "cca_ids");
    tk_ivec_t *cca_ids = tk_ivec_peek(L, -1, "cca_ids");
    lua_getfield(L, 1, "cca_dims");
    uint64_t cca_d = lua_isnil(L, -1) ? d : (uint64_t)lua_tointeger(L, -1);
    lua_pop(L, 5);
    if (cca_d > d) cca_d = d;

    uint64_t p = (uint64_t)(cca_off->n - 1);

    int64_t max_cca_uid = -1;
    for (uint64_t i = 0; i < nc; i++)
      if (chol_ids->a[i] > max_cca_uid) max_cca_uid = chol_ids->a[i];
    for (uint64_t i = 0; i < (uint64_t)cca_ids->n; i++)
      if (cca_ids->a[i] > max_cca_uid) max_cca_uid = cca_ids->a[i];

    int64_t *u2c = (int64_t *)malloc((uint64_t)(max_cca_uid + 1) * sizeof(int64_t));
    memset(u2c, -1, (uint64_t)(max_cca_uid + 1) * sizeof(int64_t));
    for (uint64_t i = 0; i < nc; i++)
      u2c[chol_ids->a[i]] = (int64_t)i;

    double *cca_Z = (double *)calloc(p * d, sizeof(double));
    double *cca_acc = (double *)malloc(m * sizeof(double));
    if (!u2c || !cca_Z || !cca_acc) {
      free(u2c); free(cca_Z); free(cca_acc);
      return luaL_error(L, "encode: CCA out of memory");
    }

    for (uint64_t t = 0; t < p; t++) {
      int64_t cs = cca_off->a[t];
      int64_t ce = cca_off->a[t + 1];
      memset(cca_acc, 0, m * sizeof(double));
      for (int64_t si = cs; si < ce; si++) {
        int64_t uid = cca_ids->a[cca_ind->a[si]];
        if (uid >= 0 && uid <= max_cca_uid) {
          int64_t cr = u2c[uid];
          if (cr >= 0)
            cblas_daxpy((int)m, 1.0, full_chol->a + (uint64_t)cr * m, 1, cca_acc, 1);
        }
      }
      cblas_dgemv(CblasRowMajor, CblasTrans,
        (int)m, (int)d, cca_wt->a[t], eigvecs->a, (int)d, cca_acc, 1, 0.0, cca_Z + t * d, 1);
    }
    free(cca_acc);
    free(u2c);

    double *cca_M = (double *)calloc(d * d, sizeof(double));
    double *cca_eig = (double *)malloc(cca_d * sizeof(double));
    double *cca_ev = (double *)malloc(d * d * sizeof(double));
    int *cca_isz = (int *)malloc(2 * cca_d * sizeof(int));
    if (!cca_M || !cca_eig || !cca_ev || !cca_isz) {
      free(cca_M); free(cca_eig); free(cca_ev); free(cca_isz);
      free(cca_Z);
      return luaL_error(L, "encode: CCA out of memory");
    }

    cblas_dgemm(CblasRowMajor, CblasTrans, CblasNoTrans,
      (int)d, (int)d, (int)p, 1.0, cca_Z, (int)d, cca_Z, (int)d, 0.0, cca_M, (int)d);
    free(cca_Z);

    int n_cca = 0;
    int cca_info = LAPACKE_dsyevr(LAPACK_ROW_MAJOR, 'V', 'I', 'U',
      (int)d, cca_M, (int)d, 0.0, 0.0, (int)(d - cca_d + 1), (int)d,
      0.0, &n_cca, cca_eig, cca_ev, (int)d, cca_isz);
    free(cca_isz);
    free(cca_M);
    if (cca_info != 0) {
      free(cca_eig); free(cca_ev);
      return luaL_error(L, "encode: CCA dsyevr info=%d", cca_info);
    }

    double *cca_V = (double *)malloc(d * cca_d * sizeof(double));
    for (uint64_t i = 0; i < d; i++)
      for (uint64_t k = 0; k < cca_d; k++)
        cca_V[i * cca_d + k] = cca_ev[i * d + (cca_d - 1 - k)];
    free(cca_ev);

    double *cca_ne = (double *)malloc(m * cca_d * sizeof(double));
    cblas_dgemm(CblasRowMajor, CblasNoTrans, CblasNoTrans,
      (int)m, (int)cca_d, (int)d, 1.0, eigvecs->a, (int)d, cca_V, (int)cca_d,
      0.0, cca_ne, (int)cca_d);

    double *orig_eig = (double *)malloc(d * sizeof(double));
    memcpy(orig_eig, eig_raw->a, d * sizeof(double));
    for (uint64_t k = 0; k < cca_d; k++) {
      double vk = 0.0;
      for (uint64_t j = 0; j < d; j++)
        vk += cca_V[j * cca_d + k] * cca_V[j * cca_d + k] * orig_eig[j];
      eig_raw->a[k] = vk;
    }
    eig_raw->n = cca_d;
    free(orig_eig);
    ...
    memcpy(eigvecs->a, cca_ne, m * cca_d * sizeof(double));
    eigvecs->n = m * cca_d;
    free(cca_ne);
