...
static inline bool tm_needs_hood_reweight (tk_graph_t *graph) {
  if (!tk_graph_weight_is_knn(graph))
    return true;
  if (graph->knn_ann_hoods || graph->knn_hbi_hoods)
    return true;
...
static inline void tm_compute_cknn_rhos (
  ...
  if (graph->knn_mode != TK_GRAPH_KNN_MODE_CKNN || !TK_GRAPH_HAS_HOODS(graph))
    return;
...
static inline void tm_add_knn (
  ...
  for (uint64_t hood_idx = 0; hood_idx < max_hoods; hood_idx++) {
    ...
    if (graph->weighted_hoods && hood_idx < graph->weighted_hoods->n) {
      ...
      tk_rvec_t *hood = graph->weighted_hoods->a[hood_idx];
      uint64_t limit = use_cknn ? hood->n : (knn < hood->n ? knn : hood->n);
      for (uint64_t j = 0; j < limit; j++) {
        if (hood->a[j].i < 0 || hood->a[j].i >= (int64_t)graph->uids_hoods->n)
          continue;
        double d_uv = hood->a[j].d;
        if (!use_cknn && d_uv > graph->knn_eps)
          break;
        ...
        tk_edge_t e = tk_edge(u, v, use_cknn ? 0.0 : d_uv);
...
static inline void tm_add_mst (
  ...
  if (candidates != NULL) {
    ...
    for (uint64_t i = 0; i < candidates->n && tk_dsu_components(graph->dsu) > 1; i ++) {
      ...
      tk_edge_t e = tk_edge(c.u, c.v, use_cknn ? 0.0 : c.w);
      khi = tk_euset_put(graph->pairs, e, &kha);
      if (!kha)
        continue;
    ...
    for (int64_t i = centers->n > 2 ? 0 : 1; i < (int64_t) centers->n; i ++) {
      ...
      double d = use_cknn ? 0.0 : tk_graph_distance(graph, u, v, graph->q_weights, graph->e_weights, graph->inter_weights);
      if (d == DBL_MAX) d = 1.0;
      tk_edge_t e = tk_edge(u, v, use_cknn ? 0.0 : d);
...
static inline void tm_add_seed_edges_immediate(
  ...
  for (uint64_t i = 0; i < n_queries; i++) {
    ...
    for (int64_t j = start; j < end; j++) {
      ...
      double d = use_cknn ? 0.0 : tk_graph_distance(graph, u, v, NULL, NULL, NULL);
      if (d == DBL_MAX) d = 1.0;
      tk_edge_t e = tk_edge(u, v, use_cknn ? 0.0 : d);
      khi = tk_euset_put(graph->pairs, e, &kha);
      if (!kha)
      ...
      if (!has_error) {
        ...
        #pragma omp critical
        {
          for (uint64_t j = 0; j < local_pairs->n; j++) {
            ...
            if (!has_error) {
              bool use_cknn = (graph->knn_mode == TK_GRAPH_KNN_MODE_CKNN);
              double d = use_cknn ? 0.0 : tk_graph_distance(graph, u, v, q_weights, e_weights, inter_weights);
              if (d == DBL_MAX) d = 1.0;
              tk_edge_t e = tk_edge(u, v, use_cknn ? 0.0 : d);
              khi = tk_euset_put(graph->pairs, e, &kha);
              if (kha) {
...
static inline void tm_compute_sigma (
  ...
  if (graph->knn_mode != TK_GRAPH_KNN_MODE_SIGMA || !TK_GRAPH_HAS_HOODS(graph) || graph->knn_alpha <= 0.0)
    return;
  graph->sigmas = tk_dvec_create(L, graph->uids->n, 0, 0);
  tk_lua_add_ephemeron(L, TK_GRAPH_EPH, Gi, -1);
  lua_pop(L, 1);
