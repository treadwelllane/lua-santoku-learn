...
test("aho", function ()
  ...
  test("case insensitive", function ()
    ...
    local ac = aho.create({ ids = ids, patterns = { "hello" } })
  ...
  test("case insensitive pattern", function ()
    local ids = ivec.create({ 1 })
    local ac = aho.create({ ids = ids, patterns = { "WORLD" } })
    local _, mids, starts, ends = ac:predict({ texts = { "hello world" } })
    assert(mids:size() == 1)
    assert(mids:get(0) == 1)
  ...
  test("diacritic text matches plain pattern", function ()
    local ids = ivec.create({ 1 })
    local ac = aho.create({ ids = ids, patterns = { "cafe" } })
    local _, mids, starts, ends = ac:predict({ texts = { "caf\xC3\xA9" } })
    assert(mids:size() == 1)
    assert(mids:get(0) == 1)
    assert(starts:get(0) == 0)
    assert(ends:get(0) == 5)
  ...
  test("diacritic pattern matches plain text", function ()
    local ids = ivec.create({ 1 })
    local ac = aho.create({ ids = ids, patterns = { "caf\xC3\xA9" } })
  ...
  test("diacritic both sides", function ()
    local ids = ivec.create({ 1 })
    local ac = aho.create({ ids = ids, patterns = { "r\xC3\xA9sum\xC3\xA9" } })
    local _, mids, starts, ends = ac:predict({ texts = { "my R\xC3\x89SUM\xC3\x89 here" } })
    assert(mids:size() == 1)
    assert(mids:get(0) == 1)
  ...
  test("position after multibyte prefix", function ()
    local ids = ivec.create({ 1 })
    local ac = aho.create({ ids = ids, patterns = { "world" } })
    local _, mids, starts, ends = ac:predict({ texts = { "h\xC3\xA9llo world" } })
    assert(mids:size() == 1)
    assert(mids:get(0) == 1)
  ...
  test("latin extended-a", function ()
    local ids = ivec.create({ 1 })
    local ac = aho.create({ ids = ids, patterns = { "scev" } })
    local _, mids, starts, ends = ac:predict({ texts = { "\xC5\xA1\xC4\x8Dev" } })
    assert(mids:size() == 1)
    assert(mids:get(0) == 1)
    assert(starts:get(0) == 0)
    assert(ends:get(0) == 6)
  ...
  test("combining marks dropped", function ()
    local ids = ivec.create({ 1 })
    local ac = aho.create({ ids = ids, patterns = { "cafe" } })
    local _, mids, starts, ends = ac:predict({ texts = { "caf\x65\xCC\x81" } })
    assert(mids:size() == 1)
    assert(mids:get(0) == 1)
    assert(starts:get(0) == 0)
    assert(ends:get(0) == 6)
  ...
  test("tag preserves diacritics in match", function ()
    local ids = ivec.create({ 1 })
    local ac = aho.create({ ids = ids, patterns = { "cafe" } })
    local result = ac:tag({ texts = { "at caf\xC3\xA9 now" }, fmt = "[%match]" })
    assert(result[1] == "at [caf\xC3\xA9] now")
  end)
